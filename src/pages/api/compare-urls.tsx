...
interface CompareRequestBody {
  ...
  compareType?: string;
  threshold?: number;
  captureAllLinks?: boolean;
}

// Utility to capture a screenshot for a given URL
const captureScreenshot = async (url: string, filename: string, clip: any = null, retries = 3): Promise<string> => {
  let attempt = 0;
...
const crawlWebsite = async (url: string, maxDepth = 2, depth = 0, visited = new Set()): Promise<string[]> => {
...
const getImageDimensions = (imgPath: string): { width: number; height: number } => {
  try {
...
const compareImages = (img1Path: string, img2Path: string, diffPath: string, threshold = 0.1): { numDiffPixels: number } => {
  try {
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    const { url1, url2, pages, compareType = 'visual', threshold = 0.1, captureAllLinks = false }: CompareRequestBody = req.body;
    ...
    try {
      ...
      res.status(500).json({ message: 'Error comparing URLs', error: errorMessage });
    }
  } else {
